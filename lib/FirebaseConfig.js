...
//     if (adminDoc.exists()) {
//       // Returning admin user data including UID
//       return { uid: currentUser.uid, ...adminDoc.data(), isAdmin: true };
//     }

//     // If not admin, fetch from regular users collection
...
//     // Returning regular user data including UID
//     return { uid: currentUser.uid, ...userDoc.data(), isAdmin: false };
//   } catch (error) {
...
export const addTeacherWithSubcollections = async (
  ...
  try {
    // Custom IDs
    const teacherId = "teacherId_1"; // Change this to your desired ID format or generate dynamically
    const subjectId = "subjectId_1";
    const taskId = "taskId_1";

    // 1. Add a new teacher document with a specific ID
    const teacherDocRef = doc(db, "teachers", teacherId);
    await setDoc(teacherDocRef, teacherData);
    console.log("Teacher added successfully with ID:", teacherId);

    // 2. Add a new subject document with a specific ID under the 'subjects' subcollection
    const subjectDocRef = doc(teacherDocRef, "subjects", subjectId);
    ...
    // 3. Add a new task document with a specific ID under the 'tasks' subcollection
    const taskDocRef = doc(subjectDocRef, "tasks", taskId);
    ...
    // Return the structure or IDs if needed
    return {
      teacherId,
      subjectId,
      taskId,
