...
//     // Create a query to fetch notes by teacherId and order by createdAt descending
//     const q = query(
//       notesCollection,
//       where("teacherId", "==", teacherId),
//       orderBy("createdAt", "desc")
...
//     // Map the results to an array of notes
//     const notes = querySnapshot.docs.map((doc) => ({
//       id: doc.id,
//       ...doc.data(),
//     }));
...
export const fetchAttendanceForTeacherAndDate = async (teacherId, date) => {
  try {
    ...
    // Query to get the attendance document for the specific date
    const dateQuery = query(attendanceRef, where("date", "==", date));
    const querySnapshot = await getDocs(dateQuery);

    const attendanceData = {};

    // Check if a document exists for the given date
    querySnapshot.forEach((doc) => {
      const data = doc.data();
      const studentAttendance = data.studentAttendance || {};

      // Add student attendance data to the response structure
      Object.keys(studentAttendance).forEach((studentId) => {
        attendanceData[studentId] = studentAttendance[studentId]; // Store attendance status
      });
    });

    return attendanceData; // Return attendance data for each student on the given date
  } catch (error) {
